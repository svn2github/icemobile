...
(function() {
    ...
    function DataTable(clientId, cfg) {
        
        var id = clientId;
        var ctr = document.getElementById(clientId);
        var orientation = cfg.orientation;
        var clientSide = cfg.clientSide;
        var properties = cfg.properties;
        
        function createDataTable(){
          
            layoutTable();
            window.addEventListener('deviceorientation', layoutTable);
            window.addEventListener('resize', layoutTable);
            //set 1st row as initial detail
            setTimeout( function(){
                setRowDetail({target: ctr.querySelector("tbody tr:first-child")});
            }, 100);
        ...
        function layoutTable(){
            ...
            ctr.style.height = (window.innerHeight - ctr.parentNode.offsetTop) + "px";

            var headersElem = ctr.querySelector("thead tr");
            var rowsElem = ctr.querySelector("tbody");
            ...
            if( horizontal ){
                ...
                detailElem.style.width = '40%';
                masterElem.style.width = '60%';
            }
            else{
                detailElem.style.height = '40%';
                masterElem.style.height = '60%';
            }
            
            //set the data region height to parent height - 25px for the headers
            masterElem.children[1].style.height = "" + (masterElem.clientHeight - 25) + "px";
            
            //new Tablesort("#{cc.id}");
            var rowSelectEvt = document.documentElement.touchStart ? 'touchEnd' : 'click';
            rowsElem.addEventListener(rowSelectEvt, setRowDetail, false);
            
            //match header and body column widths
            for (colIndex = 0; colIndex < headersElem.children.length; ++colIndex) {
                var width = rowsElem.children[0].children[colIndex].clientWidth + 'px';
                headersElem.children[colIndex].style.width = width;
                headersElem.children[colIndex].style.maxWidth = width;
            }
            //if windows, add 16px padding on right side of last header column
            if (navigator.appVersion.indexOf("Win")!=-1){
                headersElem.children[headersElem.children.length-1].style.borderRight = "16px solid transparent";
            }
        }
        
        function setRowDetail(evt){
            if( evt.target && 
               ( evt.target.nodeName != "TBODY" ) ){
                
                var prevSelected = document.querySelector('.mobi-tbl-row-sel');
                if( prevSelected ){
                    prevSelected.classList.remove('mobi-tbl-row-sel');
                }
                var tr = findTr(evt.target);
                tr.classList.add('mobi-tbl-row-sel');
                
                if( clientSide ){
                    var detailRoot = ctr.querySelector(".mobi-tbl-detail-render");
                    detailRoot.innerHTML = '';
                    
                    var renderedTemplate = ctr.querySelector(".mobi-tbl-detail-template").innerHTML;
                    var expressions = renderedTemplate.match(/\{\{[a-zA-Z0-9]*\}\}/g);
                    for( var i = 0 ; i < expressions.length ; i++ ){
                        var prop = expressions[i].substring(2,expressions[i].length-2);
                        var colIndex = properties[prop];
                        renderedTemplate = renderedTemplate.replace(expressions[i], tr.children[colIndex].innerHTML);
                    }
                    
                    detailRoot.innerHTML = renderedTemplate;
        ...
        function updateProperties(clientId, cfg){
    ...
    ice.mobi.dataTable = {
        ...
        initClient: function(clientId, cfg) {
            if (!this.dataTables[clientId]) {
                this.dataTables[clientId] = DataTable(clientId, cfg);
            } else {
                this.dataTables[clientId].updateProperties(clientId, cfg);
            }
        }
    }
